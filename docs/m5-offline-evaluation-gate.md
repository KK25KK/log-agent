...
受治理 SOP 也不在这条评测路径中。`evaluate` 直接运行 Engine/evaluation，不构造 Worker 的 `RunbookService`；因此当前数据集、门禁、版本清单和版本指纹都不包含 `RunbookGuidance`、SOP Source 版本或条目 revision。SOP 是 Worker 首次校验之后的独立后处理，不能用当前评测通过来证明知识匹配、内容治理或飞书展示质量；未来若纳入评测，必须显式扩展版本合同和数据集，而不是在现有指纹下静默加入。
...
- 真实飞书客户端交互和卡片视觉效果；
- Mock 或真实 SOP 的匹配准确性、内容有效性、owner 审批和撤销/过期治理；
- Worker 的 SOP 双重校验、飞书 SOP 纯文本展示或真实 `RunbookSource` 可用性；
- 企业历史故障上的准确率、召回率和误导率；
- 专家是否认可标签、关联阈值或下一步建议；
...
上表是该离线评测切片既有验收记录，其中的数据集/版本指纹不包含后来增加的 Worker SOP 后处理。它不能作为本轮受治理 SOP 的执行验收；SOP 的当前结果必须以独立 Mock E2E 和对应测试的实际复跑记录为准。

2026-08-24 在加入受治理 SOP 的当前工作树上重新运行 `go run ./cmd/logagent evaluate`，结果仍为 `PASSED`，数据集指纹仍为 `caf2714c80a646c5da15134c6557879565ffc8e083a66da1f1c9e49d3d0dc1f8`。这只验证 Engine/evaluation 合同未被 Worker 后处理改变；由于该数据集本身不含 SOP，不能据此宣称 SOP 检索质量已通过评测。
