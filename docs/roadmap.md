...
当前已经完成 provider-neutral 端口、确定性 Mock、Worker 后处理、严格输出/引用门禁、确定性回退、飞书有界展示、隔离的火山方舟 Responses API 适配器，以及零网络 `llm-check` 和单调用 `llm-smoke`。2026-09-01 已用专用模型级 Key 对 `doubao-seed-2-0-mini-260428` 完成一次真实合成 Smoke，并用本地 Web 完成一次 DAM 真实 SLS + 方舟真实 LLM 的同调查联合运行；认证、结构化输出、受治理真实计数 Evidence 和 Worker 应用合同通过。默认主链仍为 Mock，Prompt 审批、费用/留存策略、真实样本模型质量门禁和真实飞书 E2E 仍待真实输入。
...
- [x] 执行并归档一次 DAM 真实 SLS + 火山方舟真实 LLM 的同调查联合运行。
...
- [x] 阶段 1：受治理自然语言接单。首个切片支持 `error_spike`，先生成授权范围内的 count-only 预览，确认后才创建原有调查；Mock 垂直链路和方舟协议测试完成，真实 intent smoke 待执行。
- [x] 阶段 2：DAM TraceID 多 Logstore 查询。`trace_search_v1`、管理员资源组、8 成员 Mock、主成员优先、并发/条数/字节预算、脱敏跨库时间线、独立审计与 Checkpoint 已完成离线验收；真实 `trace-check` 已 8/8 READY、日志读取为 0，真实 `trace-smoke` 待执行。
