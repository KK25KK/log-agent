...
## 16. 2026-09-02：建立联合 RCA 评测与真实试点门禁

### 问题

普通单元测试能证明分支正确，却很容易在汇报中被误写成“根因准确率”。真实 Trace、部署记录和专家标签又不应为了赶进度而伪造或提交到仓库。因此需要一种输出自身数据边界的独立评测，同时把真实试点缺失的输入和操作步骤固化下来。

### 方案

增加 `joint-rca-evaluate` 命令和 `joint-rca-evaluation-v1` 报告。内置 Dataset 明确为 `SYNTHETIC_MOCK`，真实事故数、专家标签数、凭据、网络调用和生产声明权限都固定为零/否；Parser 拒绝未知字段以及把合成数据改标成真实数据。8 个 Case 覆盖变更支持、未变化反证、代码截断、无匹配、部署冲突、Trace 不完整、Provider 不可用和无锚点。

门禁要求 Case/状态/Verdict/引用完整性/确定性重放全部为 1，不安全确定性根因、非人工动作和外部网络调用全部为 0。另提供不含真实内容的历史 Case JSON 模板、双 Reviewer 评审单和测试环境真实联合 Smoke 手册；实际文件放在 Git 忽略的 `data/`。

### 难点与解决

- 安全文案会包含“不会确认根因”，简单关键词扫描会误报：扫描器只检查候选和动作中的正向确定性/自动处置表述，不扫描边界声明本身。
- 合成门禁可能被改成宽松阈值：阈值由代码固定，Dataset 不能提供策略；数据源或真实数量被篡改会在执行前失败。
- 没有真实 TraceID 和部署平台事实：手册明确停在预检与模板，不用本地 `HEAD`、虚构 Trace 或旧 Mock 结果冒充真实 Smoke。
- 专家对未确认事故可能被迫给根因：模板允许 `UNLABELLED/INSUFFICIENT_CONTEXT`，只对有复盘事实的 Case 计算根因类指标。

### 效果与边界

当前合成门禁为 8/8，通过率、状态、Verdict、引用完整性和确定性重放均为 1；不安全声明、自动动作和网络调用为 0。该结果只属于 `SYNTHETIC_MOCK`，真实事故数和专家标签数仍为 0。真实联调必须按 [`joint-rca-evaluation-and-real-pilot.md`](joint-rca-evaluation-and-real-pilot.md) 继续，并由人工提供代表性 TraceID、可信部署事实和评审结果。
