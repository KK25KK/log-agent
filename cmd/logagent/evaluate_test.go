...
func TestExecuteSyntheticEvaluationPassesFixedGoldenSet(t *testing.T) {
	...
	if report.Versions.PromptUsed || report.Versions.GraphVersion == "" || report.DatasetFingerprint == "" {
	...
	if metrics.MisleadingRate != 0 || metrics.ConclusiveRecall != 1 || metrics.EvidenceCoverage != 1 || metrics.CauseExactAccuracy != 1 || metrics.CauseVerdictAccuracy != 1 {
	...
	for _, result := range report.Cases {
		if !result.Passed || !result.ProductionOutputValid || !result.EvidenceContractPassed {
