...
- [ ] 至少选择 7 个高含金量主题，覆盖 Agent、Go、可靠性、LLM 和评测。
- [ ] 每项完整回答：问题、难点、方案、实现、替代方案、优势、代价、效果、后续优化。
- [ ] 对比开放式 ReAct、SDK 直连、自动重试、原始日志喂模型、只测 happy path 等备选方案。
- [ ] 每项给出仓库证据和面试追问深度，避免只罗列技术名。
- [ ] 提供至少 12 个源码/测试链接和 4 个 📦 知识框。
- [ ] 自查：所有主题都有“为什么不选别的方案”和“效果边界”；Mock/真实表述一致。
- [ ] Commit：`docs(interview): add highlights and challenges`

## Task 3：项目介绍口述稿

**文件：** 创建 `interview-kit/03-project-introduction-script.md`（180～320 行）

- [ ] 写一句话、30 秒、3 分钟、5 分钟四档脚本。
- [ ] 固定叙事：业务痛点 → 核心设计 → 调查主链 → 难点 → 真实验收 → 当前边界。
- [ ] 用“授权办案的值班工程师”“证据卷宗”“快递单号”等类比解释抽象概念。
- [ ] 输出为可直接朗读的连续中文，不写模块清单或源码讲解。
- [ ] 自查：四档信息一致、没有虚构业务指标、5 分钟稿能自然回答“项目做什么/为什么这样做/效果如何”。
- [ ] Commit：`docs(interview): add project introduction scripts`

## Task 4：简历候选条目

**文件：** 创建 `interview-kit/04-resume-bullets.md`（300～500 行）

- [ ] 生成 7 条候选，覆盖 Agent 编排、查询治理、可靠恢复、Evidence/反证、LLM、评测和真实联合 E2E。
- [ ] 每条标题尽量不超过 30 个中文字符，并给出推荐等级和适合强调的岗位能力。
- [ ] 每条补充业务背景、实现机制、方案取舍、可验证效果、技术水平、适用前提、可以说/不能说。
- [ ] 给出校招简历最终组合建议：Agent 主线版、Go 工程版、可靠性版各 2～3 条。
- [ ] 自查：标题长度脚本检查；每条至少包含做了什么、用了什么、效果是什么三项中的两项。
- [ ] Commit：`docs(interview): add resume bullet candidates`

## Task 5：逐条面试脚本

**文件：** 创建 `interview-kit/05-interview-scripts-by-resume-bullet.md`（500～900 行）

- [ ] 与 Task 4 的 7 个编号严格一一对应。
- [ ] 每条写 20～30 秒开场和 2～3 分钟完整口述稿。
- [ ] 每条按“问题—方案—效果—取舍”闭环，语言生动、逻辑连贯。
- [ ] 每条至少准备 3 个追问；全文追问不少于 21 个。
- [ ] 每条增加边界防守：Mock/真实、已实现/计划、个人贡献、生产可用性。
- [ ] 不出现具体代码、函数逐行解释或无法直接朗读的表格堆砌。
- [ ] 自查：7 个编号完整、追问 ≥21、代码围栏为 0、所有数字有证据或明确说明是样本。
- [ ] Commit：`docs(interview): add resume-aligned interview scripts`
...
- [ ] 说明五份材料的使用场景和推荐顺序。
- [ ] 给出“面试前 15 分钟”“准备 1 小时”“完整学习”三条阅读路线。
- [ ] 提醒只选择与实际贡献一致的 2～3 条简历项。
- [ ] 放置当前源码 commit、事实边界和所有文件链接。
- [ ] 自查：链接全部存在，README 不复制正文。
- [ ] Commit：`docs(interview): add interview kit index`
