...
> 状态：v0.1 已完成；v0.2 Agent 八股映射与 0→1 架构演进扩展已完成
...
- [x] 所有 17 个主题均先呈现原八股，再呈现项目映射；
- [x] 通用标准答案不依赖 Log Agent 也能成立；
- [x] 项目标准答案与 v0.1 架构、亮点和真实验收边界一致；
- [x] 至少 34 个延伸追问，并从概念逐步追到取舍与故障场景；
- [x] 外部结论均可追溯到本节列出的权威来源；
- [x] 项目映射至少提供 12 个有效 `file:///` 源码或测试链接；
- [x] 未使用的 ReAct、RAG、Multi-Agent、MCP 不被包装成已实现能力；
- [x] 业务主链路、查询治理链路和失败恢复链路均有 Mermaid 图和逐站解释；
- [x] 0→1 演进覆盖 M0～M5、真实 LLM、真实 SLS 与本地 Web 联合试点；
- [x] 每个演进阶段均回答“问题—选择—困难—验证—启示—限制”；
- [x] 至少 8 个困难按统一模板完整复盘；
- [x] 至少 8 项架构决策对比替代方案，并说明未来何时需要重选；
- [x] 所有效果均标注单测、Mock、离线评测或真实联合样本的证据层级；
- [x] `07-zero-to-one-architecture-evolution.md` 至少包含 3 张链路图、1 张演进时间线和 2 个可朗读版本；
- [x] README 增加两份新文档入口和对应阅读路线；
- [x] 行数、知识框、Mermaid、源码链接和文末速记满足阈值；
- [x] 全局链接、事实边界、术语和 Git 工作树交叉审查通过。
