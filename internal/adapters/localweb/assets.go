  ...
  <main class="shell">
    ...
    <section class="grid">
      <form id="investigation-form" class="panel form-panel">
        ...
        <label>服务<input id="service" name="service" value="order-service" maxlength="64" required></label>
        <label>环境<input id="environment" name="environment" value="prod" maxlength="64" required></label>
        <div class="two-col">
        ...
        <button class="primary" type="submit" id="submit-button">开始调查</button>
...
api("/api/meta").then(function(meta){var badges=el("mode-badges");[["SLS",meta.sls_mode],["LLM",meta.llm_mode],["飞书",meta.feishu_mode]].forEach(function(pair){var badge=document.createElement("span");badge.className="badge";var strong=document.createElement("strong");text(strong,pair[0]+" ");badge.append(strong,document.createTextNode(pair[1]));badges.appendChild(badge)});text(el("boundary"),meta.warning)}).catch(function(error){text(el("boundary"),error.message)});
})();`
