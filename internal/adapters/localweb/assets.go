...
function renderIntent(resolution){currentResolution=resolution;el("intent-preview").hidden=false;text(el("intent-status"),resolution.status+" / "+(resolution.intent||"unknown"));text(el("intent-confidence"),Math.round((resolution.confidence||0)*100)+"%");text(el("intent-scope"),(resolution.service||"-")+" / "+(resolution.environment||"-")+(resolution.trace_id_hint?" / Trace "+resolution.trace_id_hint:""));text(el("intent-window"),durationText(resolution.duration_seconds)+" / "+(resolution.template_id||"-"));var safe=resolution.status==="RESOLVED"&&(resolution.intent==="error_spike"||resolution.intent==="trace_search");el("confirm-button").disabled=!safe;text(el("intent-notice"),safe?"这是确认预览，当前尚未访问 SLS。点击确认后才会创建调查。":"当前解析结果不能启动调查（"+(resolution.reason_code||"unsupported")+"），请使用下方结构化表单修正。");}
...
function renderReport(report){var panel=el("report-panel");if(!report){panel.hidden=true;return}panel.hidden=false;text(el("outcome"),report.outcome);var summary=report.summary;el("summary-section").hidden=!summary;if(summary){text(el("summary-text"),summary.phenomenon);text(el("summary-meta"),summary.mode+" · "+summary.provider+(summary.model?" · "+summary.model:"")+" · "+summary.total_tokens+" tokens · "+summary.latency_millis+" ms")}var findings=el("findings");findings.replaceChildren();(report.findings||[]).forEach(function(item){addItem(findings,item.code,item.statement+"（置信度 "+Math.round(item.confidence*100)+"%）")});if(!findings.children.length)addItem(findings,"无确定性发现","当前证据没有形成确定性发现。");var recommendations=el("recommendations");recommendations.replaceChildren();(report.recommendations||[]).forEach(function(item){addItem(recommendations,item.code,item.statement)});if(!recommendations.children.length)addItem(recommendations,"暂无建议","当前报告没有确定性建议。");var evidence=el("evidence");evidence.replaceChildren();(report.evidence||[]).forEach(function(item){var card=document.createElement("article");card.className="evidence-card";var title=document.createElement("strong");text(title,item.name+" · "+(item.complete&&!item.truncated?"完整":"不完整"));var dl=document.createElement("dl");var pairs=item.trace_member_status?[["成员状态",item.trace_member_status],["脱敏事件",item.trace_event_count],["API 调用",item.api_calls],["扫描字节",item.processed_bytes],["耗时",item.elapsed_millisecond+" ms"],["窗口",formatTime(item.start_time)+" → "+formatTime(item.end_time)]]:[["错误数",item.error_count],["Top 错误",item.top_error||"不适用"],["API 调用",item.api_calls],["扫描字节",item.processed_bytes],["耗时",item.elapsed_millisecond+" ms"],["窗口",formatTime(item.start_time)+" → "+formatTime(item.end_time)]];pairs.forEach(function(pair){var wrap=document.createElement("div");var dt=document.createElement("dt");var dd=document.createElement("dd");text(dt,pair[0]);text(dd,pair[1]);wrap.append(dt,dd);dl.appendChild(wrap)});card.append(title,dl);evidence.appendChild(card)});var context=el("context");context.replaceChildren();if(report.trace){addItem(context,"Trace 调查",report.trace.status+"；覆盖 "+report.trace.members.length+" 个成员；共 "+report.trace.total_events+" 条脱敏事件；API "+report.trace.total_api_calls+" 次");(report.trace.events||[]).slice(0,20).forEach(function(event){addItem(context,formatTime(event.event_time)+" · "+event.member_id,(event.level?"["+event.level+"] ":"")+(event.operation?event.operation+" · ":"")+event.message)});if(report.trace.events_truncated)addItem(context,"时间线展示已截断","完整的受治理报告仍保留最多 500 条事件，页面只展示前 100 条。")}if(report.cause_status)addItem(context,"原因分析",report.cause_status+"；候选数 "+(report.cause_hypotheses||[]).length);if(report.timeline_status)addItem(context,"跨信号时间线",report.timeline_status+"；条目数 "+(report.timeline_items||[]).length);if(report.runbook_guidance)addItem(context,"SOP 人工核查",report.runbook_guidance.status+"；仅供人工核查，不会自动执行处置。");el("context-section").hidden=!context.children.length}
function renderAnchors(report){if(!report||!report.trace||!report.trace.anchor_set)return;var set=report.trace.anchor_set;var context=el("context");addItem(context,"运行时检索锚点",set.status+"；提取 "+set.anchors.length+" 个；锚点只用于精确检索，不代表根因。");(set.anchors||[]).slice(0,12).forEach(function(anchor){addItem(context,anchor.kind,anchor.file?anchor.file+":"+anchor.line+(anchor.symbol?" · "+anchor.symbol:""):(anchor.symbol||anchor.value))});el("context-section").hidden=false}
function renderCode(report){if(!report||!report.code)return;var code=report.code;var context=el("context");var deployment=code.deployment;addItem(context,"部署与代码证据",code.status+"；"+(deployment?deployment.repository_id+" @ "+deployment.commit_sha.slice(0,12):code.reason_code||"不可用")+"；只读目标部署 Commit。");(code.matches||[]).slice(0,8).forEach(function(match){addItem(context,match.file+":"+match.match_line+(match.changed_since_previous?" · 与上一部署有变更":""),match.snippet+"\n仅用于解释可能路径，不代表运行时已执行。")});if(code.diff_checked)addItem(context,"可信部署 Diff",(code.changed_files||[]).length+" 个允许范围内的变更文件；文件重叠不证明因果。");el("context-section").hidden=false}
function renderJointRCA(report){if(!report||!report.joint_rca)return;var rca=report.joint_rca;var context=el("context");addItem(context,"联合根因候选",rca.status+"；候选 "+(rca.candidates||[]).length+" 个；最高仅为 SUPPORTED_CANDIDATE，必须人工确认。");(rca.candidates||[]).slice(0,8).forEach(function(candidate){addItem(context,candidate.verdict+" · "+candidate.file+":"+candidate.line,candidate.statement+"（固定证据评分 "+Math.round(candidate.confidence*100)+"%，变更关系 "+candidate.change_relation+"）\n缺失输入："+(candidate.missing_inputs||[]).join("、"))});(rca.actions||[]).slice(0,8).forEach(function(action){addItem(context,"人工动作 · "+action.code,action.statement)});el("context-section").hidden=false}
...
api("/api/meta").then(function(meta){var badges=el("mode-badges");[["SLS",meta.sls_mode],["LLM",meta.llm_mode],["Intent",meta.intent_mode],["Code",meta.code_mode],["飞书",meta.feishu_mode]].forEach(function(pair){var badge=document.createElement("span");badge.className="badge";var strong=document.createElement("strong");text(strong,pair[0]+" ");badge.append(strong,document.createTextNode(pair[1]));badges.appendChild(badge)});text(el("boundary"),meta.warning);var enabled=meta.intent_mode&&meta.intent_mode!=="disabled";el("intent-panel").hidden=!enabled;if(enabled){api("/api/capabilities").then(function(result){var first=(result.capabilities||[])[0];if(first){el("service").value=first.service;el("environment").value=first.environment;el("template").value=first.template_id}}).catch(function(error){text(el("form-error"),error.message)})}}).catch(function(error){text(el("boundary"),error.message)});
})();`
